...
public class ExecutorTests
{
    ...
    // An AdvancedReaction runs its whole body (including dependency reads) on the executor. The
    // reaction's Context scope must carry across the executor hop, even for an executor that runs
    // the callback on its own thread without flowing ExecutionContext (DedicatedThreadExecutor) --
    // otherwise the dependency reads capture nothing, no Sources wire, and later Sets never
    // re-trigger the reaction.
    [Fact(Timeout = 10000)]
    public async Task AdvancedReaction_OnDedicatedThreadExecutor_CapturesDependencies_AndRetriggers()
    {
        using var executor = new DedicatedThreadExecutor();
        var f = new MemoFactory().AddExecutor(executor);
        ...
        await TestHelpers.WaitForConvergenceAsync(() => { lock (seen) { return seen.Contains(1); } });

        await v.Set(2); // must re-trigger: the initial run has to have wired v as a source

        await TestHelpers.WaitForConvergenceAsync(() => { lock (seen) { return seen.Contains(2); } });
        lock (seen) { Assert.Contains(2, seen); }
    }
