...
The same un-primed storm against the **lock-based engine reproduces the same permanent
staleness within seconds** (quarantined as
`RegressionTests.LockEngine_UnprimedChainUnderStorm_StrandsStale_KnownIssue`). The hole is latent
in the shipping engine; priming patterns in the existing tests masked it. Fixing the lock engine
is deliberately out of this change's scope — that protocol deserves its own careful treatment —
but the fix shape is proven here.
...
- The prototype already paid for itself: it found, reproduced, and pinned a latent
  permanent-staleness bug in the shipping engine, and proved a fix protocol for it.
