...
- Every node gets a stable, per-`Context` **id** (`SignalHandlR.Id`, allocated from the
  context's node-id slice).
- Every signal carries a **trigger** — a version counter for *value changes*:
  - `Signal.Set` bumps it **only when the value actually changes** (the value-unchanged
    short-cut notifies nobody and leaves the trigger alone — the trigger mirrors exactly what
    observers are told);
...
(`BeginStampCapture` / `RecordSourceStamp` / `TakeStampCapture`, all under `Context.Lock` — a
leaf monitor, per the lock-order rules in concurrency.md §9).
...
- structured-concurrency children read on their own flows/scopes (`ConcurrentMap` even forces
  ...
  touching the job machinery. (The one deliberately untracked case, the reaction builder's
  parallel dependency evaluations on isolated scopes, records its observed pair into the
  reaction's bucket explicitly — see `ReactionBuilder.EvaluateOnOwnScopeAsync`);
