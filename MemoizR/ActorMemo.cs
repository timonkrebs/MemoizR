...
public sealed class ActorMemo<T> : ActorValueNode<T>
{
    ...
    private async Task<Decision> ScanAndResolveAsync(Decision scan, EvaluationFrame? frame)
    {
        foreach (var source in scan.Sources!)
        {
            ...
            {
                // A throwing parent marks itself dirty; if we end up recomputing, our own read
                // of it surfaces the exception with our context.
            }
        }

        return await Actor.Run(() => ResolveScan(scan.GenSnapshot, frame)).ConfigureAwait(false);
    }
    ...
    // Clean -- IF no invalidation landed during the scan (the generation tells).
    private Decision ResolveScan(int genSnapshot, EvaluationFrame? frame)
    {
